...
func main() {
    ...
    if *cfg_dir == "" {
        ...
        // Измененная конфигурационная директория для скрытия фреймворка
            // Используем менее очевидное имя директории
            configDirNames := []string{".cache", ".local", ".config", ".data"}
            seed := make([]byte, 1)
            rand.Read(seed)
            selectedDir := configDirNames[int(seed[0])%len(configDirNames)]
            *cfg_dir = filepath.Join(usr.HomeDir, selectedDir, "session")
    }
